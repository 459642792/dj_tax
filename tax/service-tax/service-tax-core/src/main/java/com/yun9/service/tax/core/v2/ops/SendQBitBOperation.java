...
@ActionMapping(parameters = {
        @SnParameter(sn = ActionSn.send_bitb_q, taxOffice = TaxOffice.gs, cycleType = CycleType.q, area = AreaSn.shenzhen)
}
)
public class SendQBitBOperation implements TaskStartHandler2, TaxRouterBuilder {
    private final static Logger logger = LoggerFactory.getLogger(SendQBitBOperation.class);
    @Autowired
    ...
    public TaxRouterRequest build(OperationContext context) {
        ...
        return new TaxRouterRequest() {{
            ...
            setParams(new HashMap<String, Object>(5) {{
