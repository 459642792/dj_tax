...
@ActionMapping(parameters = {
        @SnParameter(sn = ActionSn.send_bita_q, taxOffice = TaxOffice.ds, cycleType = CycleType.q, area = AreaSn.shenzhen)
}
)
public class SendQDsBitAOperation implements TaskStartHandler2, TaxRouterBuilder {
    private final static Logger logger = LoggerFactory.getLogger(SendQDsBitAOperation.class);
    @Autowired
