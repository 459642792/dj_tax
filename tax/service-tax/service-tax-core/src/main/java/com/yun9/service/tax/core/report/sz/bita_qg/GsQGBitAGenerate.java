package com.yun9.service.tax.core.report.sz.bita_qg;
...
import com.yun9.service.tax.core.report.sz.bita_qg.report.*;
...
@ReportSnMapping(sns = "quanguo_q_bit_a")
public class GsQGBitAGenerate implements IReportGenerate {
    ...
    public Map<String, List<ReportDataDTO>> generate(BizTaxInstanceCategory bizTaxInstanceCategory, JsonMap body) {
        ...
        calculationValue.technologyAdmissionMatter =bizTaxInstanceCategoryBit.getTechnologyAdmissionMatter();
        calculationValue.technologySmallCompany = bizTaxInstanceCategoryBit.getTechnologySmallCompany();
        calculationValue.highTechnologyCompany = bizTaxInstanceCategoryBit.getHighTechnologyCompany();
        //==============生成报表后更新增值税信息====================
        Map<String,Object> report005 = new Report005().generate(calculationValue, history, currents).toMap();
        Map<String, Object> report001 = new Report001().generate(calculationValue, currents,clientDataMap,report005).toMap();
        bizTaxInstanceCategoryBitService.afterGeneratedReport(report001, bizTaxInstanceCategory.getId());

        return new HashMap() {{
            put("quanguo_q_bit_a_zhrmghgqysdsjdyjnssbba", ReportDataDTO.bulidReportDataDTO(report001));
            put("quanguo_q_bit_a_gdzcjszjyhmxb", ReportDataDTO.bulidReportDataDTO(new Report003().generate().toMap()));
            put("quanguo_q_bit_a_mssrjjsrsdjmdyhmxb", ReportDataDTO.bulidReportDataDTO(new Report004().generate(history).toMap()));
            put("quanguo_q_bit_a_jmsdsyhmxb", ReportDataDTO.bulidReportDataDTO(report005));
            put("quanguo_q_bit_a_jmqycgwgqyxxbgb", ReportDataDTO.bulidReportDataDTO(new Report006().generate().toMap()));
            put("quanguo_q_bit_a_jscgtzrgqysdsdynsbab", ReportDataDTO.bulidReportDataDTO(new Report007().generate().toMap()));
        }};
    }
